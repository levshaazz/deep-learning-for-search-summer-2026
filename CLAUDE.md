...
`python3 _research/check_claims.py` (+`--selftest`) · `npm run build` (must be **52 pages**) · the relevant `_audit/` gate(s) · for a `data/`/generator change: `bash _research/reproduce.sh` (byte-identity, H3). Deep context + the refactor plan/log: `_audit-report/` (`CONTEXT-ARCH-AUDIT.md`, `REFACTOR-LOG.md`).
