...
Проверки (HARD, если не сказано иное):
  ...
  [N] ЧИСЛА ПРОЗЫ — каждое число в markdown-прозе (вне ```кода```, `инлайна`, $формул$,
      ...
      из ~17 тыс. чисел совпадает почти с чем угодно — проверка была бы театром); голые годы
  ...
  [T4] КРОСС-РАНТАЙМ — если есть seminars/runs-t4/<имя>.json (T4-прогон владельца): метрики
      ...
      тайминговые (по имени: MS/TIME/SEC/DT) — WARN при расхождении больше TIMING_RATIO раз.
...
def check_notebook(name, nb_path, runs_dir, pool_data, err, warn):
...
def check_t4(name, runs_dir, t4_dir, err, warn):
    ...
    try:
        ...
        return
    timing = re.compile(r"MS|TIME|SEC|_DT|LATENCY|ELAPSED", re.I)
...
def run(sem_dir=SEM_DIR, runs_dir=RUNS_DIR, t4_dir=RUNS_T4_DIR, data_glob=DATA_GLOB,
    ...
    for nb_path in notebooks:
        ...
        residue = check_notebook(name, nb_path, runs_dir, pool_data, err, warn)
        if residue:
...
def selftest():
    ...
    case("t4-quality", True, "проза без чисел", {"SEED": 42, "NDCG": 0.6309},
         seed_code, t4={"SEED": 42, "NDCG": 0.6417})
    # 12. T4: расхождение тайминга в 3 раза — WARN, не HARD
    case("t4-timing", False, "проза без чисел", {"SEED": 42, "STAGE_MS": 100.0},
         seed_code, t4={"SEED": 42, "STAGE_MS": 300.0})
