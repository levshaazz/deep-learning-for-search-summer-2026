...
Проверки (HARD, если не сказано иное):
  ...
  [F] ЭТАЛОН ПРОГОНА — свежий дамп seminars/runs/<имя>.json сверяется с ЗАМОРОЖЕННЫМ
      seminars/runs-baseline/<имя>.json. Метрики качества сверяются СТРОГО (решение
      владельца: «качество строго, тайминги с допуском»); строгость распространяется на
      ЗАЯВЛЕННЫЕ метрики — ключи внутри RUN, куда ноутбук кладёт результат задания.
      Свежий дамп без RUN — HARD: сверять нечего, прогон делается сейчас. Эталон без RUN —
      WARN: он чинится перезаморозкой, валить на этом сборку значит наказывать за старый
      формат. Рабочие переменные, попавшие в дамп скребком globals(), — WARN; тайминговые
      (по имени: MS/TIME/SEC/DT, а также t0/t_* и overpay — см. комментарий у регекспа) —
      WARN при расхождении больше TIMING_RATIO раз. Эталона нет — шаг молча пропускается.
      Заморозка ЯВНАЯ: --freeze (перезапись существующего эталона — только --force),
      иначе очередной прогон бесшумно объявил бы эталоном сам себя.
      ЧТО ЭТОТ ШАГ НЕ ЛОВИТ: железозависимые расхождения. До 2026-09-07 здесь была
      кросс-рантаймовая сверка CPU↔T4, но она держалась на полном наборе CPU-баз, а его
      не существует и не будет — четыре захода умерли обрывом сессии, hw-rag считал одну
      ячейку 3,5 часа, полный набор стоит до суток счёта. Рантайм курса один (Colab T4),
      проза пишется по его числам — значит и эталон снимается там же.
...
Usage:  python3 _research/check_notebooks.py                    (гейт)
        ...
        python3 _research/check_notebooks.py --freeze --force --only lab-cascade
                                                                (переснять эталон ОДНОЙ тетрадки —
                                                                 той, которую действительно прогнали)
...
RUNS_BASE_DIR = os.path.join(SEM_DIR, "runs-baseline")
...
def check_notebook(name, nb_path, runs_dir, pool_data, err, warn, base_dir=None):
    ...
    alien_check("", metrics)
    if base_dir:                                # эталон грязнится ровно так же
        t4p = os.path.join(base_dir, name + ".json")
        if os.path.exists(t4p):
            try:
                alien_check(" T4", json.load(open(t4p, encoding="utf-8")).get("metrics") or {})
            except Exception:
                pass                            # нечитаемость — забота шага [F]

    # [N] числа прозы. Обосновать число может ЛЮБОЙ настоящий прогон этой тетрадки —
    # и свежий (seminars/runs), и замороженный эталон (seminars/runs-baseline). Пул из
    # обоих нужен потому, что проза могла быть написана по эталонному прогону и с тех пор
    # не менялась: искать её числа только в свежем дампе значит объявлять честную цитату
    # необоснованной на ровном месте. Сверку прогона с эталоном это не трогает — ею занят
    # шаг [F], и расхождение он ловит отдельно и строго.
    pool_dump = []
    _flatten_nums(metrics, pool_dump)
    if base_dir:
        t4_path = os.path.join(base_dir, name + ".json")
...
def check_frozen(name, runs_dir, base_dir, err, warn):
    """[F] сверка свежего дампа с ЗАМОРОЖЕННЫМ эталоном: качество строго, тайминги — WARN.

    Раньше здесь была кросс-рантаймовая сверка CPU↔T4 («качество не зависит от железа»).
    Она держалась на полном наборе CPU-баз, а его не существует и не будет: четыре захода
    умерли обрывом сессии, hw-rag считал одну ячейку 3,5 часа, полный набор стоит до суток
    счёта (решение владельца 2026-09-07 — отказаться от CPU-баз совсем). Штатный рантайм
    курса — Colab T4, проза пишется по его результатам, значит и эталон снимается там же.
    Что проверка ловит теперь: регрессию КОДА и ДАННЫХ между двумя прогонами на одном и
    том же рантайме. Чего не ловит: железозависимые расхождения — и это честно, потому что
    сравнивать их не с чем.
    """
    cur_path = os.path.join(runs_dir, name + ".json")
    base_path = os.path.join(base_dir, name + ".json")
    if not (os.path.exists(cur_path) and os.path.exists(base_path)):
        return
    tag = "F"
    loc_path, t4_path = base_path, cur_path   # loc = эталон, t4 = свежий прогон
    try:
    ...
    if "RUN" not in {k.split(".")[0] for k in t4}:
        err(f"[{tag}] {name}: в свежем дампе нет RUN — сверять заявленные метрики не с чем; "
            f"перепрогони ноутбук текущей версией (сборщик итога кладёт RUN с 2026-09-03)")
        return
    if "RUN" not in {k.split(".")[0] for k in loc}:
        warn(f"[{tag}] {name}: в замороженном эталоне нет RUN — сверять заявленные метрики "
             f"не с чем; перезаморозь эталон свежим прогоном (--freeze)")
        return
        ...
        if timing.search(k):
            ...
            if lo > 0 and hi / lo > TIMING_RATIO:
                warn(f"[{tag}] {name}.{k}: тайминг разъехался {a} ↔ {b} (> {TIMING_RATIO}×) — "
                     f"на одном рантайме так не бывает: либо другая VM, либо поехал код")
        elif a != b:
            if declared:
                err(f"[{tag}] {name}.{k}: КАЧЕСТВО разошлось с замороженным эталоном: "
                    f"{a} ↔ {b} — по решению владельца сверка строгая")
            else:
                warn(f"[{tag}] {name}.{k}: рабочая переменная разошлась {a} ↔ {b} — "
                     f"не заявлена в RUN, поэтому не HARD")
    missing = sorted(set(loc) - set(t4))
    if missing:
        warn(f"[{tag}] {name}: в свежем дампе нет метрик эталона: {', '.join(missing[:6])}"
...
def run(sem_dir=SEM_DIR, runs_dir=RUNS_DIR, base_dir=RUNS_BASE_DIR, data_glob=DATA_GLOB,
    ...
    for nb_path in notebooks:
        ...
        residue = check_notebook(name, nb_path, runs_dir, pool_data, err, warn, base_dir)
        if residue:
        ...
        check_frozen(name, runs_dir, base_dir, err, warn)
...
def selftest():
    ...
    def case(label, want_hard, md, dump_metrics, code_extra=None, baseline=None, t4=None,
        ...
        t4d = os.path.join(sem, "runs-baseline")   # эталон, с которым сверяется свежий дамп
        os.makedirs(runs)
        ...
        with contextlib.redirect_stdout(buf):
            rc = run(sem_dir=sem, runs_dir=runs, base_dir=t4d, data_glob=dglob,
                     baseline_path=bpath)
        got_hard = rc != 0
    ...
    with _ctx.redirect_stdout(_io.StringIO()):
        rc_ad = run(sem_dir=sem_x, runs_dir=os.path.join(sem_x, "runs"),
                    base_dir=os.path.join(sem_x, "t4"), data_glob=os.path.join(ddir, "*.json"),
    ...
    # 11a. строгость держится на RUN — расхождение ЗАЯВЛЕННОЙ метрики с эталоном HARD…
    ...
    # 11c. СВЕЖИЙ дамп без RUN — HARD: прогон делается сейчас, значит это недосмотр.
    #      (в фикстуре dump_metrics — свежий прогон, t4 — замороженный эталон)
    case("fresh-no-run", True, "проза без чисел", {"SEED": 42, "NDCG": 0.63}, seed_code,
         t4={"SEED": 42, "RUN": {"NDCG": 0.63}})
    # 11d. …а ЭТАЛОН без RUN — WARN: эталоном он не работает, но чинится перезаморозкой,
    #      и валить на этом сборку значило бы наказывать за старый формат дампа
    case("frozen-no-run", False, "проза без чисел", {"SEED": 42, "RUN": {"NDCG": 0.63}},
         seed_code, t4={"SEED": 42, "NDCG": 0.63})
    # 11e. совпадение с эталоном молчит — сверка не шумит на здоровом прогоне
    case("frozen-matches", False, "проза без чисел", {"SEED": 42, "RUN": {"ndcg": 0.63}},
         seed_code, t4={"SEED": 42, "RUN": {"ndcg": 0.63}})
    # 12a. t0 — абсолютный perf_counter; между VM он расходится в десятки раз, и это
    ...
    # 12b. t_index / overpay — секунды и отношение секунд, тоже тайминги
    ...
    with contextlib.redirect_stdout(io.StringIO()):
        rc = run(sem_dir=sem_e, runs_dir=os.path.join(sem_e, "runs"),
                 base_dir=os.path.join(sem_e, "t4"),
    ...
    with contextlib.redirect_stdout(io.StringIO()):
        rc1 = run(sem_dir=sem_s, runs_dir=runs_s, base_dir=os.path.join(sem_s, "t4"),
                  data_glob=os.path.join(tmp, "seed", "data", "*.json"),
                  baseline_path=bp, update=True)
        rc2 = run(sem_dir=sem_s, runs_dir=runs_s, base_dir=os.path.join(sem_s, "t4"),
                  data_glob=os.path.join(tmp, "seed", "data", "*.json"),
                  baseline_path=bp)
    ...
    print("[selftest] PASS — дамп/ассерты/SEED/выдуманное число/допуск ≈/исключения/храповик/"
          "эталон-качество-vs-тайминг/непустота все ведут себя как заявлено")
    return 0


def freeze(runs_dir=RUNS_DIR, base_dir=RUNS_BASE_DIR, force=False, only=None):
    """--freeze: заморозить текущие дампы как эталон, с которым сверяются следующие прогоны.

    Заморозка ЯВНАЯ и отказывается перезаписывать уже замороженное без --force: иначе
    очередной прогон бесшумно объявил бы эталоном сам себя, и сверка перестала бы
    что-либо ловить — ровно так храповики и превращаются в театр.

    `--only ИМЯ` переснимает эталон ОДНОЙ тетрадки. Это не удобство: перепрогнать
    обычно нужно одну, а `--freeze --force` без отбора объявил бы эталоном сам себя
    заодно и у шести остальных, которые никто не перепрогонял, — и сверка у них
    молча перестала бы что-либо значить.
    """
    os.makedirs(base_dir, exist_ok=True)
    frozen = skipped = 0
    for src in sorted(glob.glob(os.path.join(glob.escape(runs_dir), "*.json"))):
        ...
        try:
            ...
            print(f"  x {name}: дамп не читается ({e})")
            continue
        if "RUN" not in (dump.get("metrics") or {}):
            print(f"  . {name}: в дампе нет RUN — эталоном быть не может")
            continue
        ...
        with io.open(dst, "w", encoding="utf-8") as fh:
            json.dump(dump, fh, ensure_ascii=False, indent=1, sort_keys=True)
            fh.write("\n")
        frozen += 1
        print(f"  + {name}: эталон заморожен ({len(dump.get('metrics') or {})} величин)")
    print(f"[notebooks-gate] эталонов заморожено: {frozen} · пропущено: {skipped}")
    return 0
...
if __name__ == "__main__":
    ...
    if "--freeze" in sys.argv:
        ...
        sys.exit(freeze(force="--force" in sys.argv, only=_only))
