...
def main():
    ...
    # НЕ ПУТАТЬ с data/l7-real.json — это разные файлы разных генераторов. Здесь игрушечный
    # список стадий для виджета; там замеры на EnterpriseRAG-Bench из gen_l7_real.py. 07.09.2026
    # второй генератор записался поверх первого: имя l7-rag выглядело свободным, виджет остался
    # без offline/online и на пяти книжных фигурах координаты стали NaN. Ни один гейт не сказал
    # ни слова — check_claims читает только то, что ему назвали, а виджет молча рисует пустоту.
