...
SLOT_VARIANTS = {
    ...
    # Пятый слот словаря. Он существовал в chrome-ru.json, но отсутствовал в обоих реестрах —
    # то есть НЕ проверялся, а отчёт печатал «слотов: 5» (len(canon)) и создавал впечатление
    # покрытия. Вариант «Резюме ·» перечислен как исторический, между которым надпись расходилась.
    "recap-tag-prefix":      ["Итог", "Итог ·", "Резюме", "Резюме ·", "Сводка", "Сводка ·"],
}
...
SLOT_WHERE = {
    ...
    # Лимит {0,160} отсекал не шум, а сами слайды: RU+EN содержимое заголовка/кикера в 11 и 19
    # фрагментах из 69 длиннее 160 символов, и там слот молча не проверялся. Границей служит
    # закрывающий тег, а не длина; [\s\S] нужен, потому что разметка переносится по строкам.
    "misconception-h2":      (r'<h2[^>]*>([\s\S]{0,600}?)</h2>', "misconception"),
    "misconception-kicker":  (r'class="slide-kicker"[^>]*>([\s\S]{0,600}?)</span>\s*</span>', "misconception"),
    "payoff-divider-prefix": (r'<h2[^>]*>([\s\S]{0,600}?)</h2>', "divider"),
    "recap-tag-prefix":      (r'class="def-tag"[^>]*>([\s\S]{0,600}?)</span>', None),
}
