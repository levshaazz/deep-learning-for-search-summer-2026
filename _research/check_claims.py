...
ERBRANK  = load(DATA, "l7-real.json")            # frozen open-corpus subset (ERB, MIT): retrieve MRR@10 0.4142 → rerank 0.4963 (rerank helps)
...
def l7_book_claims():
    ...
    return [
        ...
        C("L7 mm retrMrr",   ERBRANK["retrieve"]["mrrAt10"], r"retrieval <strong>MRR@10 = ([\d.]+)</strong>"),
        C("L7 mm rerankMrr", ERBRANK["rerank"]["mrrAt10"],   r"rises to <strong>([\d.]+)</strong>"),
...
_COV_YEAR_TAIL = re.compile(r'[.,](?:19|20)\d\d(?!\d)')    # «…2026» сразу за числом ⇒ это дата, а не дробь
...
def _coverage_uncovered(html, gated):
    out = set()
    vis = _coverage_visible(html)
    for m in _COV_DEC.finditer(vis):
        s = m.group()
        # ДАТА С ГОДОМ. _COV_DEC откусывает от «13.10.2026» первые две группы — «13.10», —
        # и гейт годами читал бы это как число 13,10. Прежняя защита _COV_DATE смотрела
        # только на ведущий ноль («03.06»), поэтому 08.09.2026 проходила, а 13.10.2026 нет:
        # шесть дек упали на титульных датах, добавленных вместе с календарём. Смотрим не на
        # само число, а на то, что идёт СЛЕДОМ: год после второй точки — признак даты, и
        # настоящая дробь так не выглядит. Календарные даты проверяет шаг [K] — сверкой с
        # расписанием, а не диапазоном, — так что непокрытыми они не остаются.
        if _COV_YEAR_TAIL.match(vis, m.end()):
            continue
...
# ── [K] КАЛЕНДАРЬ: даты на слайдах == calendar в data/course.json ─────────────────────────
#    Класс дефекта, ради которого проверка написана. Календаря как данных не существовало
#    до 08.09.2026: даты стояли руками на слайде дедлайнов вводной лекции, на титулах дек и
#    в src/lib/midterm.js. Все три копии разошлись и с расписанием, и между собой — слайд
#    нёс дедлайны прошлого потока (июнь–июль), титулы трёх дек — 03.06.2026, а мидтерм был
#    объявлен неделей 4, хотя в расписании стоит в седьмой. Ни один гейт не сказал ни слова,
#    потому что число, ни с чем не связанное, не с чем и сверять.
#    Теперь источник один (calendar.start), и проверка ловит расхождение слайда с ним.
def calendar_checks(report, text):
    import datetime as _dt
    course = json.loads((DATA / "course.json").read_text(encoding="utf-8"))
    cal = course.get("calendar")
    if not cal:
        report.append(("HARD", "calendar: в data/course.json нет блока calendar"))
        return
    start = _dt.date.fromisoformat(cal["start"])
    sched = course["schedule"]

    # 1. Даты строк расписания выведены из старта, а не вписаны руками.
    bad = [r["week"] for r in sched
           if r.get("date") != (start + _dt.timedelta(weeks=r["week"] - 1)).isoformat()]
    if bad:
        report.append(("HARD", f"calendar: даты недель {sorted(set(bad))} не выводятся из "
                               f"calendar.start — перегони _research/gen_calendar.py"))
    else:
        report.append(("OK", f"calendar: {len(sched)} строк расписания выведены из старта "
                             f"{cal['start']} ✓"))

    # 2. Титул каждой деки показывает дату СВОЕГО занятия.
    row_of = {}
    for r in sched:
        for n in r.get("lectures", []):
            row_of[n] = r
    parts = ROOT / "Lectures"
    seen = miss = 0
    for sub in sorted(parts.iterdir()):
        deck = sub.name
        f = sub / "parts" / "01-title.html"
        if not f.is_file():
            continue
        try:
            num = int(deck.split("-")[0])
        except ValueError:
            continue
        row = row_of.get(num)
        if not row:
            continue
        m = re.search(r'Date\s*</div>\s*<div class="meta-value">(.*?)</div>',
                      f.read_text(encoding="utf-8"), re.S)
        if not m:
            continue
        seen += 1
        d = _dt.date.fromisoformat(row["date"])
        want = f"{d.day:02d}.{d.month:02d}.{d.year}"
        got = re.sub(r"\s+", " ", m.group(1)).strip()
        if got != want:
            miss += 1
            report.append(("HARD", f"calendar({deck}): титул показывает {got!r}, "
                                   f"а занятие {want} (неделя {row['week']})"))
    if seen and not miss:
        report.append(("OK", f"calendar: даты на титулах {seen} дек(и) == расписание ✓"))

    # 3. Слайд дедлайнов вводной лекции == assignmentsSchedule.
    intro = parts / "00-introduction" / "parts" / "15-assignments.html"
    if intro.exists():
        html = intro.read_text(encoding="utf-8")
        RU = ["янв", "фев", "мар", "апр", "мая", "июн", "июл", "авг",
              "сен", "окт", "ноя", "дек"]
        gone = []
        for a in course.get("assignmentsSchedule", []):
            due = _dt.date.fromisoformat(a["due"])
            token = f"{due.day} {RU[due.month - 1]}"
            if token not in html:
                gone.append(f"{a['label']['ru']} → {token}")
        if gone:
            report.append(("HARD", "calendar: на слайде дедлайнов нет дат — "
                                   + "; ".join(gone)))
        else:
            report.append(("OK", f"calendar: {len(course.get('assignmentsSchedule', []))} "
                                 f"дедлайнов на слайде == данные ✓"))

    # 4. Мидтерм: неделя и дата в src/lib/midterm.js == расписание.
    mj = ROOT / "src" / "lib" / "midterm.js"
    mid = next((r for r in sched if r["kind"] == "checkpoint"), None)
    if mid and mj.exists():
        js = mj.read_text(encoding="utf-8")
        d = _dt.date.fromisoformat(mid["date"])
        RU = ["января", "февраля", "марта", "апреля", "мая", "июня", "июля",
              "августа", "сентября", "октября", "ноября", "декабря"]
        need_ru = f"Неделя {mid['week']}"
        need_day = f"{d.day} {RU[d.month - 1]}"
        if need_ru not in js or need_day not in js:
            report.append(("HARD", f"calendar(midterm.js): ждали «{need_ru}» и «{need_day}» "
                                   f"— мидтерм стоит в неделе {mid['week']} ({mid['date']})"))
        else:
            report.append(("OK", f"calendar: мидтерм в midterm.js == расписание "
                                 f"(неделя {mid['week']}, {need_day}) ✓"))
...
def main():
    ...
    report = []
    calendar_checks(report, text)                   # [K] даты слайдов == calendar
