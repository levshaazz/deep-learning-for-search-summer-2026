...
def bench():
    ...
    return {
        ...
        # ModernBERT — что стало с энкодером за шесть лет после BERT. Числа дословно из
        # Warner et al., arXiv:2412.13663 (§2 «Architecture» и аннотация): не вычисляются здесь.
        # Смысл для лекции: архитектура BERT осталась, а сменились ПОЗИЦИИ (RoPE), активация
        # (GeGLU), схема внимания (локальное окно + глобальный слой каждый третий) и работа
        # с паддингом — и это дало 8192 контекста вместо 512 при сравнимом размере.
        "modernbert": {
            "cite": "warner-2024-modernbert",
            "baseLayers": 22, "baseParamsM": 149, "baseHidden": 768, "baseGluExpansion": 2304,
            "largeLayers": 28, "largeParamsM": 395, "largeHidden": 1024, "largeGluExpansion": 5248,
            "nativeSeqLen": 8192,
            "trainTokensT": 2,
            "globalEveryNthLayer": 3,
            "localWindowTokens": 128,
            "ropeThetaGlobal": 160000,
            "ropeThetaLocal": 10000,
            # Разбор руками для слайда-механизма: во что обходится длина входа и что
            # возвращает чередование окна с глобальным слоем. Считается здесь, а не на слайде.
            "byHand": {
                "fullPairs8192": 8192 * 8192,
                "bertPairs512": 512 * 512,
                "ratioFullToBert": (8192 * 8192) // (512 * 512),
                "windowPairs": 8192 * 128,
                "ratioFullToWindow": (8192 * 8192) // (8192 * 128),
                "globalLayers": 22 // 3,
                "localLayers": 22 - 22 // 3,
                "mixedPairsPerPass": (22 // 3) * 8192 * 8192 + (22 - 22 // 3) * 8192 * 128,
                "allFullPairsPerPass": 22 * 8192 * 8192,
                "savingX": round(22 * 8192 * 8192 / ((22 // 3) * 8192 * 8192 + (22 - 22 // 3) * 8192 * 128), 1),
            },
            "_doc": "ModernBERT: RoPE вместо абсолютных позиций, GeGLU, чередование локального "
                    "окна 128 токенов с глобальным вниманием каждый третий слой, unpadding "
                    "на обучении и инференсе; нативные 8192 токена, 2 трлн токенов обучения.",
        },
        # mmBERT — многоязычная ветка того же поколения (Marone et al., arXiv:2509.06888).
        # Для трёхъязычного курса важен приём: 1700+ низкоресурсных языков подмешаны ТОЛЬКО
        # в фазе затухания, и этого хватает, чтобы резко поднять на них качество.
        "mmbert": {
            "cite": "marone-2025-mmbert",
            "trainTokensT": 3,
            "languages": 1800,
            "lowResourceAddedInDecay": 1700,
            # Разбор руками: 1700 из 1800 — это 94 % языков, которых модель увидела ТОЛЬКО
            # в фазе затухания; оставшиеся 100 — ровно то покрытие, что у прежнего поколения.
            "byHand": {
                "highResource": 1800 - 1700,
                "lowResourceSharePct": round(1700 / 1800 * 100, 1),
                "evenSplitTokensM": round(3e12 / 1800 / 1e6),
            },
            "_doc": "mmBERT: 3 трлн токенов, более 1800 языков; обратное расписание доли "
                    "маскирования и обратная температура сэмплирования; низкоресурсные языки "
                    "добавлены только в фазе затухания.",
        },
    }
