...
def run(root=ROOT, data=DATA, style=STYLE, listing=False):
    ...
    if not terms:
        ...
        return 1
    # Запись без `ban` — это НЕ дыра, а зафиксированное решение «регекспом не проверяется».
    # Часть строк §7 механически в запрет не переводится: «пакет» и «расплата» — обычные слова,
    # законные в своём прямом смысле («пакетная обработка», «расплата за длину» — денежная
    # метафора, которую §7 прямо разрешает там, где речь о цене); «поиск vs извлечение» и
    # «терм» — выбор слова, а не запрет чего-либо. Раньше такие строки давали голый WARN
    # «не гейтится», и понять из него, забыли запись или её невозможно написать, было нельзя.
    # Теперь у каждой такой строки есть запись с причиной — решение видно, а не подразумевается.
    for t in terms:
        if not t.get("why", "").strip():
            what = f"запрет «{t['ban']}»" if t.get("ban") else f"решение «{t.get('canon')}»"
            errors.append(f"[P] {what} без обоснования — его нельзя ни оспорить, ни снять")
    ...
    if table is None:
        ...
        # [S2] ОБРАТНАЯ сверка. Докстринг этого гейта обещает «и наоборот», но её не было: строка
        # §7 с запрещёнными вариантами, не заведённая в JSON, не тревожила ничего — то есть
        # решение, записанное в документе, оставалось непроверяемым. Ровно то расхождение
        # документа и проверки, ради которого гейт и написан.
        # style_table() уже возвращает ОЧИЩЕННЫЙ канон строки (без разметки), поэтому искать
        # «**жирный**» здесь бессмысленно — первая же версия этой проверки молча не срабатывала
        # ни разу: строки не совпадали с шаблоном, цикл пропускал их все. Проверено удалением
        # термина из JSON — гейт промолчал.
        # Строка §7 считается покрытой, если ХОТЬ ОДИН канон JSON встречается в ней: строки таблицы
        # бывают составными («трансформер, внимание (attention), запрос/ключ/значение» — одна строка
        # про целую группу), и требовать дословного равенства значит ловить форму, а не расхождение.
        canon_heads = [t["canon"].split("/")[0].strip().lower() for t in terms]
        for row in table:
            head = str(row).strip().lower()
            if head and not any(c and c in head for c in canon_heads):
                # WARN, а не HARD, и вот почему. Часть запретов §7 нельзя перевести в регексп
                # механически: «бит» запрещён только в роли структурной единицы главы (а как
                # единица информации — базовый термин курса), «пакет» и «перепись» — обычные
                # слова, «recall» законен внутри имени метрики recall@10. Такой запрет требует
                # решения автора, а не догадки гейта. Поэтому строка печатается как НЕПРОВЕРЯЕМАЯ:
                # молчать о ней нельзя (тогда решение §7 просто не существует для проверки),
                # но и валить сборку за отсутствие регекспа, которого никто не написал, — тоже.
                warns.append(f"[S2] решение §7 «{row}» не переведено в проверку "
                             f"(нет записи в narrative/glossary-ru.json) — оно не гейтится")
    ...
    for f in files:
        ...
        for t in terms:
            if not t.get("ban"):
                continue          # решение без регекспа — см. [P] и комментарий у ban=None
