/* vlmfinal-book.mjs — drive each target Book widget through every step via
   window.__figs[beat].setStep(k), both themes, screenshot the widget root.
   Usage: node vlmfinal-book.mjs <05|06> <light|dark> */
...
const lang = 'en';
const outdir = join(ROOT, '_internal/l56_vlm_final', `book-${chapter}-${theme}`);
...
const page = await b.newPage({ viewport:{width:1280,height:920}, deviceScaleFactor:1.5 });
...
// enumerate widgets present (beat -> maxStep), in the order figs registered
...
for (const {beat,max} of figs){
  for (let k=0;k<=max;k++){
    // scroll the widget's graphic into view, set the step, screenshot the root
