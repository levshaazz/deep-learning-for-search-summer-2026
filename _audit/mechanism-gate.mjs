...
if (process.argv[1] && process.argv[1].endsWith('mechanism-gate.mjs')) {
  if (process.argv.includes('--selftest')) {
    ...
    // Храповик надо СНАЧАЛА вооружить. Отсутствующий файл — это process.exit(1) внутри
    // loadBaseline (защита от «rm baseline && --update-baseline», закрытая по следам viz-probe),
    // поэтому селфтест, начинавший с невооружённого прогона, умирал на первой же строке и
    // не печатал ни одного [OK]. Гейт при этом был исправен — сломан был его собственный тест,
    // и падал он только в CI, где --selftest вообще запускают.
    quiet(() => run({ root: tmp, baselinePath: bp, update: true }));   // первичная запись легальна
