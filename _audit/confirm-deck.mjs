/* confirm-deck.mjs — capture target deck slides (L5 s36 PCA-rotation, L6 s37 archflow,
   L6 s38 layernorm) at every step, both themes, on PORT 9131. */
...
const OUT  = join(fileURLToPath(new URL('../', import.meta.url)), '_internal/l56_vlm_confirm/deck');
const PORT = 9131;
...
const TARGETS = [
  { deck:'05-dl-embeddings-dimred.html',                 slide:36, max:4, tag:'pca-rotation' },
  { deck:'06-contextual-attention-transformers.html',    slide:37, max:6, tag:'archflow' },
  { deck:'06-contextual-attention-transformers.html',    slide:38, max:3, tag:'layernorm' },
];
...
for (const theme of ['light','dark']){
  for (const deck of Object.keys(byDeck)){
    ...
    const base = basename(deck,'.html').slice(0,2);
    for (const t of byDeck[deck]){
      ...
      for (let s=0; s<=t.max; s++){
        ...
        const f = join(OUT, `${base}-s${String(t.slide).padStart(2,'0')}-${t.tag}-step${s}-${theme}.png`);
        await page.screenshot({path:f});
        process.stdout.write(`${base} s${t.slide} step${s} ${theme} (label="${cur.label}" dom=${cur.step}/${cur.max})  `);
      }
      console.log('');
    }
