...
export const mountRankingMetrics = defineWidget({
  ...
  render({ host, data, labels, el }) {
    ...
    // Подпись MRR — ДВУМЯ строками, по той же причине, что и лид MAP ниже: одной строкой
    // русский текст даёт 574 px при рамке 557 и уходит за правый край. На macOS он влезал,
    // на ubuntu CI — нет (шрифты шире), поэтому дефект был виден только там: viz-probe OOB,
    // шаги 2 и 3, дека 05 s49. Ширину подписи решает самый широкий язык, а не тот, на
    // котором её писали.
    sub('mrr', panel.x, tableBottom + 16,
      labels.mrrHint || 'first relevant result;');
    sub('mrr', panel.x, tableBottom + 32,
      labels.mrrHintB || 'MRR averages 1/rank over many queries');

    // — step 3: MAP
    const mapY = tableBottom + 56;   // +16: подпись MRR заняла вторую строку
