    {
      ...
      body: {
        ...
        ru: [
          "Вот как нить учится, кого слушать. Из каждого слова я сучу на станке три коротких вектора. *Query* (запрос) — что я ищу? *Key* (ключ) — что я предлагаю другим? И *Value* (значение) — что я на самом деле передам, если ты обратишь на меня внимание. Три обучаемые проекции, \\(Q=XW_Q,\\ K=XW_K,\\ V=XW_V\\), и всё.",
          "Чтобы оценить нить \\(i\\) против нити \\(j\\), беру скалярное произведение её запроса с чужим ключом: \\(E_{ij}=Q_i\\cdot K_j\\). Высоко — когда совпадают, низко — когда нет. Softmax по строке превращает скоры в веса с суммой единица — это *мягкий* lookup, а не жёсткий выбор одного ключа, — а выход есть смесь значений, \\(Y_i=\\sum_j A_{ij}V_j\\). Нить слушает понемногу всех, но больше всего того, кто ей подходит.",
          "Одна загвоздка. В \\(d_k\\) измерениях дисперсия скалярного произведения растёт *с размерностью* — большие скоры толкают softmax в пик, почти one-hot, и градиент гаснет. Поэтому сначала делю на \\(\\sqrt{d_k}\\). Та же похожесть, но числа мягче: без масштаба \\(\\mathrm{softmax}(0,0,6)\\to(0{,}002,0.002,0.995)\\); после деления на \\(\\sqrt{4}=2\\) — \\(\\mathrm{softmax}(0,0,3)\\to(0{,}045,0.045,0.909)\\). По-прежнему решительно, но полотно остаётся *мягким* — достаточно, чтобы учиться дальше.",
        ],
        tt: [
