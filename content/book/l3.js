    ...
    { id: 'climb-rrf', kind: 'scrolly', widget: 'rrf-fusion', data: 'l3-rrf' },
    {
      ...
      body: {
        en: [
          ...
          "That sounds circular, and it is, in the productive way. Picture a bored web surfer clicking links at random forever; the fraction of time they spend on each page, in the long run, *is* that page's PageRank. You compute it by **power iteration**: start everyone equal at 1/3 ≈ 0.333, let each page hand its score out evenly along its outgoing links, repeat, and the numbers settle. Run it on a tiny three-page web — A points to B, B to C, C to both A and B — and watch the votes flow. B is the only page with *two* in-links (from A and from C), so it accumulates the most: after the first pass it already jumps to about 0.475, and as the iterations settle it converges to **≈0.40** (0.397 to be exact). A and C split the remainder — A lands near 0.215, C near 0.388 — and B holds the largest share precisely because two pages, not one, vote for it. One catch worth a footnote: a page with no outgoing links would be a black hole that hoards everyone's score, so the surfer is allowed to randomly teleport (with probability about 0.15) to keep the whole thing from collapsing.",
