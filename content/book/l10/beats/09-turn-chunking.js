    {
      ...
      body: {
        ...
        ru: [
          "Прежде чем поиск найдёт отрывок, его должен был нарезать **чанкинг**. Корпус режется на куски фиксированного размера, каждый эмбеддится и индексируется — и извлечь можно только *целый чанк*. Значит, нарезка решает, что вообще находимо: если ответ разрезан между двумя чанками, ни одно извлечение не вернёт его целиком. **Чанк Норрис** рубит свиток на равные отрывки; **Токенозавр** (лекция 2) напоминает, что куски — это суб-словные токены, а не символы.",
          "Та же фигура `rag-pipeline` возвращается — теперь со стадией **chunk→embed** в фокусе, входной стадией, что решает всё ниже по течению. Ручки — **size** и **overlap**, и следующий подъём делает их эффект точным.",
        ],
        tt: [
          ...
          "Шул ук `rag-pipeline` фигурасы кайта — хәзер **chunk→embed** баскычы фокуста, аскы агымдагы бар нәрсәне хәл итүче керү баскычы. Борычлар — **size** һәм **overlap**, ә киләсе менү аларның тәэсирен төгәл итә."
        ],
      },
    },
