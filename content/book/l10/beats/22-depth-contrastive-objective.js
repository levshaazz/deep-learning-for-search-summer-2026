    {
      ...
      body: {
        en: [
          "Where do these models come from? A Scout is trained with a **contrastive objective**, InfoNCE: \\(\\mathcal{L} = -\\log \\frac{e^{\\,\\mathrm{sim}(q,d^+)/\\tau}}{\\sum_d e^{\\,\\mathrm{sim}(q,d)/\\tau}}\\). It is a softmax over similarities that pulls the query toward its positive document and pushes it away from the negatives, scaled by a temperature \\(\\tau\\) — exactly DPR's negative log-likelihood of the positive, and the same InfoNCE from Lecture 7.",
