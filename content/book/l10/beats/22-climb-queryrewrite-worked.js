    {
      ...
      body: {
        ...
        ru: [
          ...
          ":::calc **HyDE** (один истинный документ). Голый запрос ставит истинный документ на \\(8\\)-е → recall@5 \\(= 0\\), реципрокный ранг (RR) \\(= 1/8 = \\mathbf{0{,}125}\\). HyDE пишет гипотетический ответ, эмбеддит *его*, и истинный документ поднимается на ранг \\(2\\) → recall@5 \\(= 1\\), RR \\(= 1/2 = \\mathbf{0{,}5}\\). **Multi-query** (отдельный gold-set из \\(5\\) релевантных). Один запрос достаёт \\(2/5\\) → recall@5 \\(= \\mathbf{0{,}4}\\); объединение \\(3\\) перефразировок — \\(4/5\\) → recall@5 \\(= \\mathbf{0{,}8}\\). :::",
