    {
      ...
      body: {
        ...
        ru: [
          ...
          ":::calc **HyDE** (один истинный документ). Голый запрос ставит истинный документ на \\(8\\)-е → recall@5 \\(= 0\\), реципрокный ранг (RR) \\(= 1/8 = \\mathbf{0{,}125}\\). HyDE пишет гипотетический ответ, кодирует *его*, и истинный документ поднимается на ранг \\(2\\) → recall@5 \\(= 1\\), RR \\(= 1/2 = \\mathbf{0{,}5}\\). **Multi-query** (отдельный gold-set из \\(5\\) релевантных). Один запрос достаёт \\(2/5\\) → recall@5 \\(= \\mathbf{0{,}4}\\); объединение \\(3\\) перефразировок — \\(4/5\\) → recall@5 \\(= \\mathbf{0{,}8}\\). :::",
          "HyDE закрывает разрыв лексики, ища текстом в форме ответа; multi-query расширяет покрытие, спрашивая одно и то же по-разному. Оба поднимают полноту, не трогая ретривер — просто подают ему запрос получше.",
        ],
        tt: [
          "Ике яңадан язу, ике төрле өстәмә — ике төрле gold-set та үлчәнгән, беркайчан бутамыйча.",
          ":::calc **HyDE** (бер чын документ). Ялангач сорау чын документны \\(8\\) нчегә куя → recall@5 \\(= 0\\), реципрок ранг (RR) \\(= 1/8 = \\mathbf{0{,}125}\\). HyDE гипотетик җавап яза, *аны* эмбеддлый, һәм чын документ \\(2\\) нче рангка күтәрелә → recall@5 \\(= 1\\), RR \\(= 1/2 = \\mathbf{0{,}5}\\). **Multi-query** (\\(5\\) релевантлы аерым gold-set). Бер сорау \\(2/5\\) ала → recall@5 \\(= \\mathbf{0{,}4}\\); \\(3\\) парафразаның берләшмәсе \\(4/5\\) ала → recall@5 \\(= \\mathbf{0{,}8}\\). :::",
