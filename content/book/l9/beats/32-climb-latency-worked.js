    {
      ...
      body: {
        ...
        ru: [
          ...
          ":::calc клиент→API \\(3\\) + эмбеддинг запроса \\(8\\) + ANN-поиск по \\(10^6\\) квантованным векторам \\(12\\) + переранжирование топ-100 кросс-энкодером \\(60\\) + кэш и лог (async) \\(2\\) + ответ \\(4\\) \\(= \\mathbf{89}\\) мс, уверенно под SLA \\(200\\) мс. *Точное* сканирование \\(10^6 \\times 768\\)-d векторов стоило бы \\(\\approx 520\\) мс на одном шаге поиска — оно само пробивает бюджет. Тёплое попадание в кэш возвращается за \\(\\approx 5\\) мс. :::",
        ...
        tt: [
          ...
          ":::calc клиент→API \\(3\\) + сорауны эмбеддлау \\(8\\) + \\(10^6\\) квантланган вектор буенча ANN эзләве \\(12\\) + кросс-энкодер белән топ-100 яңадан ранжлау \\(60\\) + кэш һәм лог (async) \\(2\\) + җавап \\(4\\) \\(= \\mathbf{89}\\) мс, \\(200\\) мс SLA астында иркен. \\(10^6 \\times 768\\)-d векторларны *төгәл* сканлау бары эзләү адымы өчен генә \\(\\approx 520\\) мс торыр иде — ул бюджетны үзе үк ярып чыга. Җылы кэш эләгүе \\(\\approx 5\\) мс та кайта. :::",
