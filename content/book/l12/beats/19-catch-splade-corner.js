    {
      ...
      body: {
        ...
        ru: [
          "Есть подвох. Расширение добавляет термины в запрос, а значит более длинные постинг-листы для обхода — поэтому запрос SPLADE может быть *медленнее* голого BM25, хотя живёт в том же индексе. Хуже того, затраты часто диктует *горстка* очень частых терминов расширения — почти-стоп-слова и подслова с огромными постинг-листами, — поэтому одной разрежённости мало, и реальные деплои отсекают по документной частоте или по порогу влияния. А если выученные веса станут слишком плотными, преимущество инвертированного индекса исчезает совсем.",
