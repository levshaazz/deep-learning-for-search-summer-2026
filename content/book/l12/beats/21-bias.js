    {
      ...
      body: {
        ...
        ru: [
          "Смещение входит в RAG-систему на **двух стадиях** и накапливается. **Поиск** наследует корпус: перекошенный корпус выносит перекошенные отрывки — порог полноты из Оракула, теперь этическая проблема. **Генерация** наследует модель: LLM усиливает всё, чему её научило обучение. Поисковик выбирает смещённые свидетельства; генератор облекает их в беглую прозу.",
          "Мультимодальность делает это **острее**. Общее пространство CLIP справедливо ровно настолько, насколько справедливы его обучающие пары изображение–подпись — предел, что мы только что отметили, — так что смещённое распределение пар становится смещённым пространством поиска.",
          "Смягчение операционно, а не декларативно: **аудируй корпуса; измеряй результаты по подгруппам; диверсифицируй источники.** Смещение, что не измеряешь, ты выкатываешь в продакшн. Суть — сделать справедливость *метрикой, которую отслеживаешь*, как Оракул сделал метрикой верность, — а не надеждой, которую держишь.",
        ],
        tt: [
