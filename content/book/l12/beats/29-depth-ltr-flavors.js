    {
      ...
      body: {
        en: [
          "There are three ways to teach an order. **Pointwise** scores each \\((q, d)\\) pair alone and sorts by the score — which is exactly the cross-encoder from Lecture 10. **Pairwise** learns the relative question \"is \\(d_i\\) better than \\(d_j\\)?\" — this is RankNet. **Listwise** optimizes the *whole* list directly toward the metric, like ListNet, ListMLE, and (by weighting gradients with \\(\\Delta\\)nDCG) LambdaMART.",
        ...
        ru: [
          "Есть три способа научить порядку. **Поточечный** (pointwise) оценивает каждую пару \\((q, d)\\) отдельно и сортирует по оценке — это ровно кросс-энкодер из лекции 10. **Попарный** (pairwise) учит относительный вопрос «\\(d_i\\) лучше \\(d_j\\)?» — это RankNet. **Списочный** (listwise) оптимизирует *весь* список прямо под метрику, как ListNet, ListMLE и (взвешивая градиенты на \\(\\Delta\\)nDCG) LambdaMART.",
