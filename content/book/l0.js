...
export default {
  ...
  beats: [
    ...
    },
    {
      ...
      imgCaption: {
        en: 'The arc of the course: keywords → meaning → vectors → RAG. The signposts are the syllabus.',
        ru: 'Арка курса: ключевые слова → смысл → векторы → RAG. Указатели — это программа.',
      },
      body: {
        en: [
    ...
    { id: 'turn-the-map', kind: 'scrolly', widget: 'course-map' },
    {
      ...
      imgCaption: {
        en: 'The arc as stepping stones — each lecture is one stone, and you cross by stepping on the last.',
        ru: 'Арка как камни-ступени — каждая лекция один камень, и переходишь, опираясь на предыдущий.',
      },
      body: {
        en: [
          "Seven weeks, and the climb is left to right. We start classical: TF-IDF and BM25, inverted indexes, and the ranking metrics (nDCG, MAP, MRR, Recall@k) you'll use to tell a good system from a bad one. This is the baseline and the vocabulary — skip it and the neural stuff is just magic words. Then we go neural: word and contextual embeddings that turn meaning into coordinates, transformers and attention, and bi- and cross-encoders that learn what's relevant instead of just counting matched words. Then we make it survive scale: approximate nearest-neighbor search (HNSW, IVF, PQ), FAISS, vector databases, and the unglamorous production work — quantization, caching, latency — that decides whether your system is usable. Finally we make it answer: RAG, its evaluation (which is harder than it sounds — how do you grade a confident-sounding answer?), and agentic loops that critique and correct themselves.",
        ...
        ru: [
          'Семь недель, и подъём идёт слева направо. Начинаем с классики: TF-IDF и BM25, инвертированные индексы и метрики ранжирования (nDCG, MAP, MRR, Recall@k), которыми вы будете отличать хорошую систему от плохой. Это база и словарь — пропустите их, и нейросетевая часть превратится в волшебные слова. Потом уходим в нейросети: словные и контекстные эмбеддинги, превращающие смысл в координаты, трансформеры и внимание, би- и кросс-энкодеры, которые учатся релевантности, а не просто считают совпавшие слова. Потом учим это выживать в масштабе: приближённый поиск ближайших соседей (HNSW, IVF, PQ), FAISS, векторные базы и негламурную продакшн-работу — квантизацию, кэширование, латентность, — которая и решает, можно ли вашей системой пользоваться. Наконец, учим это отвечать: RAG, его оценку (а она сложнее, чем кажется — как поставить балл уверенно звучащему ответу?) и агентные циклы, которые сами себя критикуют и исправляют.',
