...
export default {
  ...
  beats: [
    ...
    { id: 'turn-the-map', kind: 'scrolly', widget: 'course-map' },
    {
      ...
      body: {
        en: [
          "Seven weeks, and the climb is left to right. We start classical: TF-IDF and BM25, inverted indexes, and the ranking metrics (nDCG, MAP, MRR, \\(\\text{Recall@}k\\)) you'll use to tell a good system from a bad one. This is the baseline and the vocabulary — skip it and the neural stuff is just magic words. Then we go neural: word and contextual embeddings that turn meaning into coordinates, transformers and attention, and bi- and cross-encoders that learn what's relevant instead of just counting matched words. Then we make it survive scale: approximate nearest-neighbor search (HNSW, IVF, PQ), FAISS, vector databases, and the unglamorous production work — quantization, caching, latency — that decides whether your system is usable. Finally we make it answer: RAG, its evaluation (which is harder than it sounds — how do you grade a confident-sounding answer?), and agentic loops that critique and correct themselves.",
