    {
      ...
      body: {
        ...
        ru: [
          'Канонический реранкер — кросс-энкодер **monoBERT** (Nogueira и Cho 2019): подайте \\([\\text{CLS}]\\,q\\,[\\text{SEP}]\\,d\\,[\\text{SEP}]\\) в BERT, прочтите \\([\\text{CLS}]\\) через линейный слой, получите оценку релевантности. **monoT5** (Nogueira и др. 2020) делает то же на T5, выдающем токен «true» или «false». Это **поточечные** реранкеры (оценивают по одному документу); **попарные** и **списочные** варианты сравнивают документы. На практике маленький кросс-энкодер **ms-marco-MiniLM** — рабочая лошадка-Судья, достаточно быстрый, чтобы переранжировать сотню кандидатов в жёстком бюджете задержки.',
        ],
        tt: [
          'Канон реранкер — кросс-энкодер **monoBERT** (Nogueira &amp; Cho 2019): \\([\\text{CLS}]\\,q\\,[\\text{SEP}]\\,d\\,[\\text{SEP}]\\) BERT’ка бир, \\([\\text{CLS}]\\)’ны сызыклы катлау аша укы, релевантлык бәясе ал. **monoT5** (Nogueira һ.б. 2020) шуны ук T5 белән эшли, ул «true» яки «false» токены чыгара. Болар — **нокта波** реранкерлар (берәр документ бәяли); **парлы** һәм **исемлекле** вариантлар документларны чагыштыра. Практикада кечкенә кросс-энкодер **ms-marco-MiniLM** — эшче Судья, йөз кандидатны тар вакыт бюджетында кабат тәртипкә салырлык тиз.',
        ],
      },
    },
