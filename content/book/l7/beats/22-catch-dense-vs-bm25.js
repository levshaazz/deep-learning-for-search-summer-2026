    {
      ...
      body: {
        ...
        ru: [
          'Один честный твист напоследок. Соблазнительно думать, что обученный плотный извлекатель всегда бьёт старый BM25 — нет. На zero-shot бенчмарке **BEIR**, усреднённо по 18 внедоменным датасетам, **BM25 даёт nDCG@10 = 0.43, а одновекторный denseDPR лишь 0.38**: лексический поиск выигрывает вне домена (модель поздней интеракции вроде ColBERTv2 возвращается к 0.5). Внутри домена всё переворачивается — на MS MARCO MRR@10 у denseDPR 0.33 бьёт 0.187 у BM25. Один плотный вектор забывает слишком много; лечение — **гибридный** поиск и богатая поздняя интеракция, это Лекция 8.',
        ],
        tt: [
