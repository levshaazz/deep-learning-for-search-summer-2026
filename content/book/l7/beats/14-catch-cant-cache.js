    {
      ...
      body: {
        ...
        ru: [
          'Соблазнительное решение: «раз предподсчёт векторов документов ускорил Разведчика, почему не предпосчитать и оценки Судьи?» Нельзя. Вектор би-энкодера — свойство **только документа**, он одинаков для каждого запроса и кэшируется идеально. Оценка кросс-энкодера — свойство **пары** \\((q,d)\\): смени запрос — сменится оценка — значит, нет ничего независимого от запроса, что можно хранить. Это и есть глубокая причина, почему кросс-энкодером нельзя *извлекать*, только *переранжировать* список, извлечённый кем-то ещё.',
        ],
        tt: [
