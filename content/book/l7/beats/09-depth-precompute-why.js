    {
      ...
      body: {
        ...
        ru: [
          'Вот весь аргумент скорости для Разведчиков. Раз башня документа не видит запрос, каждый документ кодируется **один раз, офлайн**, и векторы хранятся. На запросе вы кодируете запрос **один раз** и делаете \\(N\\) дешёвых скалярных произведений — не \\(N\\) прогонов энкодера. Один проход плюс проход по векторам против миллиона проходов энкодера. Этот разрыв и делает поиск вообще возможным, и готовит следующий скачок: даже \\(N\\) произведений можно сделать сублинейными приближённым поиском соседей (Лекция 9).',
        ],
        tt: [
