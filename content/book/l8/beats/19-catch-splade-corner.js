    {
      ...
      body: {
        ...
        ru: [
          "Есть подвох. Расширение добавляет термины в запрос, а значит более длинные постинг-листы для обхода — поэтому запрос SPLADE может быть *медленнее* голого BM25, хотя живёт в том же индексе. Хуже того, затраты часто диктует *горстка* очень частых терминов расширения — почти-стоп-слова и субслова с огромными постинг-листами, — поэтому одной разрежённости мало, и реальные деплои отсекают по document frequency или по impact-порогу. А если выученные веса станут слишком плотными, преимущество инвертированного индекса исчезает совсем.",
