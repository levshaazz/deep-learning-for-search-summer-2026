    {
      ...
      body: {
        ...
        ru: [
          ...
          "Сырой, набранный человеком запрос проваливается тремя разными способами, и назвать, который из них у тебя, — и есть всё мастерство. Есть **лексический разрыв** — смысл верный, слова не те. Есть **разрыв конкретности** — запрос взят не на той высоте: слишком конкретен, чтобы попасть в принцип, или слишком расплывчат, чтобы попасть в факт. И есть **разрыв композициональности** — один вопрос, чьи улики лежат в *разных* пассажах, которых ни один поиск не соберёт за раз.",
          "Один инструмент закрывает все три: языковая модель, перекраивающая запрос до того, как он дойдёт до двери. Но каждая перекройка — минимум один лишний **вызов LLM**: задержка, деньги и свежая поверхность, на которой модель может нафантазировать. Переписывание — высокая отдача, но никогда не бесплатно. Поэтому настоящее ремесло — это *диагноз*: прочти провал, назови разрыв и потянись за тем единственным росчерком, что ему подходит.",
        ],
        tt: [
