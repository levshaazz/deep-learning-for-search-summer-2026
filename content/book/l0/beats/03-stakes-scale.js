    {
      ...
      body: {
        en: [
          "Two numbers to set the mood — order-of-magnitude figures, not precise counts. The web is on the order of \\(10^{12}\\) pages — a trillion. And every single day, a large fraction of queries are ones the engine has never seen before — Google has long quoted [around 15%](https://blog.google/products/search/our-latest-quality-improvements-search/) — brand-new phrasings, fresh news, typos no one anticipated. Sit with that for a second. You cannot pre-compute the answers, because a meaningful fraction of tomorrow's questions don't exist yet. And you cannot read everything when the user hits enter, because reading a trillion pages takes far longer than the quarter-second a user will wait before deciding your product is broken.",
        ...
        ru: [
          'Два числа для настроения — порядки величины, а не точные подсчёты. Веб — порядка \\(10^{12}\\) страниц, триллион. И каждый день заметная доля запросов — те, которых движок никогда раньше не видел: Google давно называет [около 15%](https://blog.google/products/search/our-latest-quality-improvements-search/) — новые формулировки, свежие новости, опечатки, которых никто не предвидел. Задержись на этом на секунду. Ответы не предвычислить, потому что заметная доля завтрашних вопросов ещё попросту не существует. И нельзя прочитать всё в момент, когда пользователь нажимает Enter, потому что прочитать триллион страниц куда дольше, чем те четверть секунды, что пользователь готов ждать, прежде чем решит, что твой продукт сломан.',
          'Поэтому оба наивных плана умирают сразу. План А — «ответить на каждый запрос заранее» — гибнет на тех 15%, которых ты никогда не видел. План Б — «сканировать весь корпус на каждый запрос» — гибнет по часам: даже по микросекунде на документ триллион документов — это больше недели вычислений на один поиск. Грубая сила не работает ни так, ни эдак. Это не сноска — это и есть причина, по которой существует вся область.',
