  ...
  beats: [
    {
      ...
      imgCaption: {
        en: 'No shared symbols yet. Before meaning, we need a common alphabet — and then a way to measure when two utterances are close.',
        ru: 'Пока нет общих символов. Прежде чем добраться до смысла, нужен общий алфавит — а потом способ измерить, когда две фразы близки.',
      },
      body: {
        en: [
    ...
    },
    {
      ...
      imgCaption: {
        en: 'Tokenosaurus: never an unknown word again. Unseen words still split into pieces we have seen.',
        ru: 'Токенозавр: больше никаких неизвестных слов. Невиданное слово всё равно распадается на знакомые куски.',
      },
      body: {
        en: [
    ...
    { id: 'stats-zipf-heaps', kind: 'scrolly', widget: 'zipf-heaps', data: 'l2-corpus-stats' },
    {
      ...
      imgCaption: {
        en: '(1,1) and (10,10) point the same way. Euclidean distance √162 ≈ 12.73 screams "different"; the direction is identical.',
        ru: '(1,1) и (10,10) смотрят в одну сторону. Евклидово расстояние √162 ≈ 12,73 вопит «разные»; направление при этом одинаково.',
      },
      body: {
        en: [
          "Tokenization done, every phrase is now a vector of numbers — a point in space, where each coordinate counts (roughly) how much of some feature the phrase carries. To compare two phrases, the obvious move is to measure the straight-line distance between their points: ‖a − b‖, the Euclidean distance you learned in school. Obvious… and wrong.",
          "Here's the trap, with exact numbers. Take the phrase whose vector is **a = (1, 1)** and the same phrase said twice as emphatically, **b = (10, 10)**. They carry the *same* meaning — both point in exactly the same direction, up-and-to-the-right at 45°. But the Euclidean distance between them is ‖a − b‖ = √(9² + 9²) = **√162 ≈ 12.73**. Raw distance screams \"these are totally different!\" — when in fact one is just a louder version of the other.",
        ...
        ru: [
          'Токенизация позади — теперь каждая фраза это вектор чисел, точка в пространстве, где каждая координата считает (грубо), сколько в фразе того или иного признака. Чтобы сравнить две фразы, очевидный ход — измерить расстояние по прямой между их точками: ‖a − b‖, евклидово расстояние из школы. Очевидно… и неверно.',
          'Вот ловушка, с точными числами. Возьми фразу с вектором **a = (1, 1)** и ту же фразу, сказанную вдвое напористее, **b = (10, 10)**. Смысл у них *один и тот же* — обе смотрят ровно в одну сторону, вправо-вверх под 45°. Но евклидово расстояние между ними ‖a − b‖ = √(9² + 9²) = **√162 ≈ 12,73**. Сырое расстояние вопит: «они совершенно разные!» — хотя на деле одна просто громче другой.',
    ...
    },
    {
      ...
      imgCaption: {
        en: 'Sir Cosine: relevance is a small angle. Divide out both lengths and only direction survives.',
        ru: 'Сэр Косинус: релевантность — это маленький угол. Подели на обе длины — и выживет только направление.',
      },
      body: {
        en: [
          "Enter **Sir Cosine and the Knights of the Unit Sphere**. Their whole creed: forget distance, measure the *angle* between two vectors. The instrument is **cosine similarity** — the cosine of that angle, written cos θ = (a · b) / (‖a‖ ‖b‖). The numerator is the dot product, Σ aᵢbᵢ; the denominator divides out *both* lengths, so the magnitude cancels and only direction is left. Point the same way → angle 0 → cosine 1 (as similar as it gets). At right angles → cosine 0 (nothing in common). Opposite → cosine −1. For ordinary word-count vectors, which can't go negative, the score lives in [0, 1].",
          "Run it on our trap from the last beat: a = (1, 1), b = (10, 10). Dot product = 1·10 + 1·10 = 20; ‖a‖ = √2, ‖b‖ = √200; cosine = 20 / (√2·√200) = 20 / √400 = 20 / 20 = **1.0**. Where Euclidean said 12.73 (\"miles apart\"), Sir Cosine says 1.0 (\"identical\"). He saw through the loudness to the meaning.",
          "And here's the elegant payoff that makes vector search practical. Expand the squared distance: ‖x − y‖² = ‖x‖² + ‖y‖² − 2(x · y). If you first **normalize** every vector to unit length — push every point onto the surface of the unit sphere — that collapses to ‖x − y‖² = 2(1 − cos θ). On the sphere, cosine, dot product, and (negative) Euclidean distance all rank neighbours in the *exact same order*. That's why real systems L2-normalize their embeddings and then run a blazing-fast dot-product index: they get cosine's meaning at a dot product's speed. The next figure is the worked example on the sphere — watch the numbers, no jokes.",
        ],
        ru: [
          'На сцену выходят **Сэр Косинус и Рыцари Единичной Сферы**. Весь их девиз: забудь про расстояние, измеряй *угол* между двумя векторами. Инструмент — **косинусная близость**, косинус этого угла: cos θ = (a · b) / (‖a‖ ‖b‖). В числителе скалярное произведение, Σ aᵢbᵢ; знаменатель делит на *обе* длины, так что величина сокращается и остаётся только направление. Смотрят в одну сторону → угол 0 → косинус 1 (максимально похожи). Под прямым углом → косинус 0 (ничего общего). Противоположно → косинус −1. Для обычных векторов счётчиков слов, которые не уходят в минус, оценка живёт в [0, 1].',
          'Прогоним на нашей ловушке из прошлого бита: a = (1, 1), b = (10, 10). Скалярное произведение = 1·10 + 1·10 = 20; ‖a‖ = √2, ‖b‖ = √200; косинус = 20 / (√2·√200) = 20 / √400 = 20 / 20 = **1,0**. Там, где евклид сказал 12,73 («за тридевять земель»), Сэр Косинус говорит 1,0 («одно и то же»). Он разглядел за громкостью смысл.',
          'И вот изящная расплата, которая делает векторный поиск практичным. Разверни квадрат расстояния: ‖x − y‖² = ‖x‖² + ‖y‖² − 2(x · y). Если сперва **нормировать** каждый вектор к единичной длине — вытолкнуть каждую точку на поверхность единичной сферы — это схлопывается в ‖x − y‖² = 2(1 − cos θ). На сфере косинус, скалярное произведение и (отрицательное) евклидово расстояние ранжируют соседей в *точно том же порядке*. Поэтому реальные системы L2-нормируют свои эмбеддинги, а потом гоняют молниеносный индекс по скалярному произведению: они получают смысл косинуса на скорости скалярного произведения. Следующая фигура — разобранный пример на сфере; следи за числами, без шуток.',
        ],
      },
    },
    ...
    { id: 'catch-curse-highd', kind: 'scrolly', widget: 'highd-histogram', data: 'l2-highd' },
    {
      ...
      imgCaption: {
        en: 'The Wraith: in high dimensions distance loses contrast. Concentration is only his first trick — hubness and anisotropy are the other two.',
        ru: 'Призрак: в высоких размерностях расстояние теряет контраст. Концентрация — лишь первый трюк; хабность и анизотропия — два других.',
      },
      body: {
        en: [
        ...
        ru: [
          ...
          '**Анизотропия.** Этот трюк уже про сами эмбеддинги, а не про число размерностей. Сырые представления из модели вроде BERT или GPT-2 не заполняют пространство равномерно — они жмутся в узкий **конус**, так что даже два не связанных токена уже стоят под малым углом, и сырой косинус докладывает обо всём как о «довольно похожем». Твоя прекрасная мера угла раздувается и теряет хватку. Лекарство — толкнуть облако обратно к изотропии: отбеливание, стандартизация или, лучше всего, контрастивное обучение, которое нарочно стягивает синонимы и расталкивает самозванцев. Практический урок: никогда не доверяй сырому косинусу на «из коробки» скрытых состояниях, не откалибровав их сперва.',
        ],
      },
    },
    {
