    {
      ...
      body: {
        ...
        ru: [
          "В реальном лексическом конвейере нормализация сидит внутри аналайзера. На индексации цепочка идёт токенизация → нижний регистр → лемма/стем → удаление стоп-слов, так что в индекс попадают только нормализованные термы. На запросе работает *тот же* аналайзер — иначе запрос *котята* никогда не совпадёт с индексированной леммой *котёнок*, и вы тихо пересобрали промах.",
          "В Elasticsearch или OpenSearch это одно объявление: встроенный аналайзер *russian* (стеммер Snowball) или hunspell/лемма-плагин, привязанный к полю один раз, чтобы управлять и индексацией, и поиском. Ему не нужен GPU, он детерминирован и переносит ранговый переворот из toy прямо в прод.",
          "Такова форма всего лечения для лексического поиска: правило- или словарно-основанная нормализация, дешёвая и симметричная, применённая одним кодом к обоим плечам системы.",
        ],
        tt: [
