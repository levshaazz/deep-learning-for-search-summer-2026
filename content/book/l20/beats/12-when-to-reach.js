    {
      ...
      body: {
        en: [
          ...
          "And for production search, the best Russian baseline is neither alone but the **hybrid**: lemmatized BM25 fused with a multilingual dense retriever by RRF (from L8). The lexical arm keeps the exact terms a dense vector blurs — names, product codes, legal citations — while the dense arm carries meaning past the morphology. Two failure modes, two arms, one fused ranking.",
        ...
        ru: [
          "Свяжем всё вместе. Для keyword-поиска по точным термам — **BM25 с лемматизацией**, дёшево, прозрачно и чинит формы. Для семантики, синонимов и парафраза — **мультиязычный плотный** энкодер, смысл поверх морфологии, без лемматизатора. Для узкого русского домена — **ru-специфичная** модель или дообучение. Для кросс-язычного ru↔en — **мультиязычный вектор** или LaBSE, одно пространство для двух языков.",
          "А для прод-поиска лучший русский baseline — не одно из двух, а **гибрид**: лемматизированный BM25, слитый с мультиязычным плотным ретривером через RRF (из L8). Лексическое плечо держит точные термы, которые плотный вектор размывает — имена, коды, юридические ссылки, — а плотное несёт смысл поверх морфологии. Два режима отказа, два плеча, одно слитое ранжирование.",
        ...
        tt: [
          ...
          "Ә прод-эзләү өчен иң яхшы рус baseline — икесеннән берсе түгел, ә **гибрид**: RRF (L8\'дән) аша мультител плотный retriever белән кушылган лемматизацияле BM25. Лексик җилкә плотный вектор юкартучы төгәл термнарны — исемнәр, кодлар, юридик сылтамалар — тота, ә плотный морфология аша мәгънәне йөртә. Ике җимерелү режимы, ике җилкә, бер кушылган ранжирлау.",
