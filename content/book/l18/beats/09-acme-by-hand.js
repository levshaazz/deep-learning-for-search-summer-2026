    {
      ...
      body: {
        ...
        ru: [
          "Подойди к моему читальному столу. Я раскладываю одну крошечную записку про компанию ACME, и капитан подаёт свой запрос: *acme revenue growth*. Сначала делаю наивно — рву записку на чанки и оцениваю каждую оторванную страницу отдельно. Страница, в которой и лежит ответ, гласит: *it highlighted 3% growth*. Но *it* — местоимение, а имя ACME стояло двумя предложениями выше, на странице, которую я уже оторвал. Косинус до запроса — вялые \\(0{,}5164\\).",
          "А тем временем пустая обманка — чанк, где буквально написано *revenue growth* и не сказано ни про кого конкретно, — набирает \\(0{,}5774\\) и встаёт на первое место. Мой золотой ответ остаётся на втором: его обходит обманка, у которой с запросом общие поверхностные слова, но не смысл. Ответ лежит прямо на столе, а поиск проходит мимо, потому что золотой чанк не может доказать, что он про ACME.",
          "Теперь сначала прочти всю записку целиком, *а потом* клади линейку вдоль корешка и усредняй. Внимание по всему документу успевает влить «ACME-ность» в это одинокое *it* ещё до того, как я отмечу хоть одну границу. Косинус золотого чанка поднимается до \\(0{,}7071\\) и выходит на первое место; обманка проваливается назад. Тот же текст, тот же запрос, тот же индекс — а порядок просто переворачивается. Этот один переворот и есть вся лекция, измеренная числом.",
        ],
        tt: [
