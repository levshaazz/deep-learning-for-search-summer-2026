...
Все дампы ложатся в `seminars/runs/`, журналы — в `seminars/runs/logs/<имя>.log`.
Развилки «CPU сюда, ускоритель туда» больше нет: полного набора CPU-баз не существует и не
будет (решение владельца 2026-09-07 — четыре захода умерли обрывом сессии, полный набор стоит
до суток счёта), поэтому штатный рантайм курса один — Colab T4, и сверять свежий прогон нужно
с ЗАМОРОЖЕННЫМ эталоном того же рантайма (`seminars/runs-baseline/`, шаг [F] в
check_notebooks). Эталон обновляется только явной командой `check_notebooks.py --freeze`,
чтобы очередной прогон не объявил эталоном сам себя.
...
def target_dir(payload):
    """Каталог для дампа. Он теперь один: рантайм у курса один, разводить нечего."""
    return RUNS_CPU
...
def selftest():
    import shutil
    import tempfile
    global RUNS_CPU, LOGS, ROOT, known_notebooks
    tmp = tempfile.mkdtemp(prefix="import_runs_selftest_")
    keep = (RUNS_CPU, LOGS, ROOT, known_notebooks)
    ...
    check("CPU-дамп лёг в runs", os.path.exists(os.path.join(RUNS_CPU, "nb.json")))
    # 2. повтор идемпотентен
    took, same, sk = import_runs([make_zip("nb.zip", payload())], log=quiet_log.append)
    check("повтор ничего не переписывает", took == 0 and same == 1)
    # 3. свежий прогон ЗАМЕНЯЕТ предыдущий дамп — каталог один, история живёт в git,
    #    а сверку с прошлым состоянием ведёт замороженный эталон (шаг [F]), не этот скрипт
    ...
    after = open(os.path.join(RUNS_CPU, "nb.json"), encoding="utf-8").read()
    check("свежий прогон лёг в runs", os.path.exists(os.path.join(RUNS_CPU, "nb.json")))
    check("свежий прогон заменил прежний дамп", after != before)
    ...
    RUNS_CPU, LOGS, ROOT, known_notebooks = keep
